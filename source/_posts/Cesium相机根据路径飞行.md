...
# 前言
Cesium相机根据路径飞行，参考官网例子：[CZML Path](https://sandcastle.cesium.com/?src=CZML%20Path.html)
...
[
  ...
  },
  {
    ...
    "position": {
      ...
      "cartographicDegrees": [
        0, 114.11929057321021, 22.508639454137754, 0, 1200, 114.13791536061427,
        22.492303116519146, 0
      ]
    }
  }
]
...
相机位置，第一个参数是时间（clock.multiplier乘秒），第二个到第四个参数分别是时间所在的经度，纬度，高度
```javascript
"position": {
  "cartographicDegrees": [
    0, 114.11929057321021, 22.508639454137754, 0, 1200, 114.13791536061427,
...
const startFly = (path) => {
  viewer.dataSources
    .add(Cesium.CzmlDataSource.load(path))
    .then((dataSource) => {
      let flyEntity = dataSource.entities.getById("path");
      // 设置初始角度
      flyEntity.viewFrom = new Cesium.Cartesian3(200, 0, 200);
      // 避免相机视角卡顿
...
[Cesium 视角跟随-第一人称视角漫游](https://blog.csdn.net/linzi19900517/article/details/129089787)